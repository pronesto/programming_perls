...
double get_checksum(const vector<float>& C) {
  double sum = 0;
  for (float val : C) {
    // Use double to prevent precision loss during summation
    sum += (double)val;
  }
  return sum;
}
...
int main() {
  ...
  report("Naive (ijk)", duration, get_checksum(C));
  ...
  duration = chrono::duration<double, milli>(end - start).count();
  report("Optimized (ikj)", duration, get_checksum(C));
  ...
  for (int ts : tile_sizes) {
    ...
    duration = chrono::duration<double, milli>(end - start).count();
    report("Tile Size " + to_string(ts), duration, get_checksum(C));
  }
